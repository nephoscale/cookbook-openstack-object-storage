...
action :ensure_exists do
  ...
  req.each do |params|
    ...
    if ::File.exist?(device)
      ...
      status = Mixlib::ShellOut.new("xfs_admin -l #{device}").run_command.status
      output = Mixlib::ShellOut.new("xfs_admin -l #{device}").run_command.stdout
      Chef::Log.debug("xfs_admin status #{status} - #{output}")

      case params[:type]
      when 'xfs'
        Chef::Log.debug("Running xfs_admin -l #{device}")
        if not Mixlib::ShellOut.new("xfs_admin -l #{device}").run_command.exitstatus == 0
          Chef::Log.debug("Running mkfs.xfs -f -i size=512 #{device}")
