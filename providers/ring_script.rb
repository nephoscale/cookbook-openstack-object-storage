...
def generate_script # rubocop:disable Metrics/AbcSize
  ...
  ['account', 'container', 'object'].each do |which|
    ...
    disk_state = search(:node, "chef_environment:#{node.chef_environment} AND roles:#{role}")
  ...
  ['account', 'container', 'object'].each do |which|
    ...
    disk_data[which].keys.sort.each do |ip|
      disk_data[which][ip].keys.sort.each do |uuid|
        ...
        if new_disks[which].key?(v['mountpoint'])
          s << "swift-ring-builder #{ring_path}/#{which}.builder add r#{v['region']}z#{v['zone']}-#{v['ip']}:#{ports[which]}/#{v['mountpoint']} #{v['size']}\n"
          must_rebalance = true
        end
      end
    end
